...
namespace LIAL_MATRIX
{
    ...
    private:
        ...
        void init(size_t i, size_t j, Type init_value)
        {
            ...
            matrix_ptr = vector<vector<Type>>(i, *vectorPtr);
            delete vectorPtr;
        }
    ...
    public:
        ...
        {
            msg("ctor");
            matrix_ptr = vector<vector<Type>>(1, init_row);
        }

        Matrix(initializer_list<initializer_list<Type>> init_matrix)
        {
            msg("ctor");
            matrix_ptr = vector<vector<Type>>(init_matrix.begin(), init_matrix.end());
        }

        Matrix(vector<vector<Type>>& init_vector)
        {
            msg("ctor");
            matrix_ptr = vector<vector<Type>>(init_vector.begin(), init_vector.end());
        }
        ...
        Matrix<Type> operator/(const Type scalar) const
        {
            Matrix<Type> result = Matrix<Type>(matrix_ptr);
            for (size_t i = 0; i < this->get_i(); i++)
                for (size_t j = 0; j < this->get_j(); j++)
                    (*result)[i][j] /= scalar;
            return result;
        ...
        Matrix operator*(const Matrix& matrix) const
        {
            if (this->get_i() != matrix.get_i() || this->get_j() != matrix.get_j())
                throw "d1";
            Matrix<Type> result(matrix.get_i(), matrix.get_j(), 0);
        ...
        Matrix sub(size_t si, size_t sj) const
        {
            ...
            return new Matrix(sub_matrix_ptr);
